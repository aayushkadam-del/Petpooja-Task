...
import { User, Mail, Phone, Calendar, Lock, CheckCircle, AlertCircle } from 'lucide-react';
import db from '../db/db';
import { useNavigate } from 'react-router-dom';
import { Button } from "@/components/ui/button";

// Presentational input field moved outside main component to keep identity stable
const InputField = ({ icon: Icon, name, type, placeholder, label, value, onChange, onBlur, touched, error, isValid }) => {
...
const SelectField = ({ icon: Icon, name, label, value, onChange, onBlur, touched, error, options }) => {
  const hasError = touched && error;
    ...
    setErrors(prev => {
      ...
      switch (fieldName) {
        case 'name':
          ...
          else if (!patterns.name.test(value)) newErrors.name = 'Name must be 3-50 characters and contain only letters';
          else delete newErrors.name;
          break;

        case 'email':
          if (!value) newErrors.email = 'Email is required';
          else if (!patterns.email.test(value)) newErrors.email = 'Please enter a valid email address';
          else delete newErrors.email;
          break;

        case 'phone':
          if (!value) newErrors.phone = 'Phone number is required';
          else if (!patterns.phone.test(value)) newErrors.phone = 'Phone number must be exactly 10 digits';
          else delete newErrors.phone;
          break;

        case 'age':
          if (!value) newErrors.age = 'Age is required';
          else if (!patterns.age.test(value)) newErrors.age = 'Age must be between 1 and 120';
          else delete newErrors.age;
          break;

        case 'password':
          if (!value) newErrors.password = 'Password is required';
          else if (!patterns.password.test(value)) newErrors.password = 'Password must be at least 8 characters with uppercase, lowercase, number, and special character';
        ...
        <div style={{
          ...
          {successMessage && (
            <div style={{
              ...
              <CheckCircle size={20} color="#155724" style={{ marginTop: '2px', flexShrink: 0 }} />
              <p style={{
                fontSize: '14px',
                color: '#155724',
                margin: 0,
            ...
            <ul style={{
              ...
              {passwordChecks.map((check, index) => (
                <li key={index} style={{
                  ...
                  alignItems: 'center',
                  gap: '8px',
                  marginBottom: index < passwordChecks.length - 1 ? '4px' : '0'
                }}>
                  <div style={{
                    width: '6px',
                    height: '6px',
